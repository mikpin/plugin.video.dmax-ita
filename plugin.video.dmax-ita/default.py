...
_regex_extractNumbers = re.compile("<li class=\"number\">(.*?)</li>",re.DOTALL);
_regex_extractNumber = re.compile("(.*?)>([0-9]*)<(.*?)",re.DOTALL);
_regex_extractListMedium = re.compile("<ol class=\"list medium\">(.*?)</ol>",re.DOTALL);
_regex_extractEpisode = re.compile("<li class=\"item\">(.*?)</li>", re.DOTALL);
_regex_extractEpisodeLink = re.compile("<a class=\"link\" href=\"(.*)\" ");
_regex_extractEpisodeTitle = re.compile("<h3 class=\"title\">(.*?)</h3>");
...
_regex_extractItaVideoIds = re.compile("<param name=\"@videoPlayer\" value=\"(.*)\" />");
...
def mainPage():
    ...
    for show in shows:
        ...
        show_link = show.group(2) + "altri-video/"
...
def showPage(link):
    global thisPlugin
	#baseLink
    page = load_page(urlShows + link)
    pageCount = 0
    numbers = list(_regex_extractNumbers.finditer(page))
    for number_html in numbers:
        number = _regex_extractNumber.search(number_html.group(1)).group(2)
        pageCount += 1
    if pageCount == 0:
        pageCount=1
    ...
    for i in range(1, pageCount + 1, 1):
        if i > 1:
            page = load_page(urlShows + link + "?pg=" + str(i))
        listmedium = _regex_extractListMedium.search(page).group(1)
        #print listmedium.group(
        episodes = list(_regex_extractEpisode.finditer(listmedium))
        ...
        for episode in episodes:
            ...
            print repr((episod_title,episode_link))
...
def showEpisode(link):
    ...
    videoIds = list(_regex_extractItaVideoIds.finditer(page));
    playlistContent = []
    x = 0
